...
// //Checks for type of event that is a Pull Request
//   if (githubData[i].type === 'PullRequestEvent') {
//     //adds the user who created the pull request to pullUsers array
//     pullUsers.push(githubData[i].payload.pull_request.user.login); debugger;
//     //Checks to see if pull request has been closed (approved) and who completed the merge. 
//   } if (githubData[i].payload.action === 'closed' && githubData[i].payload.pull_request.merged_by.login === 'stevebrownlee') {
//     //Adds the pull user to steveApproved array
//     steveApproved.push(githubData[i].payload.pull_request.user.login);
//   }
// }
// //ES6 way to sort unique items in array (https://medium.com/front-end-hacking/getting-unique-values-in-javascript-arrays-17063080f836)
// steveApproved = [...new Set(steveApproved)];
// console.log(steveApproved);

//Answer key
console.log("1. How many total commits were made in all of Steve's events?")

function commitCounter () {
  let totalCommits = 0;

  for (let i = 0; i < githubData.length; i++) {
    if (githubData[i].payload.hasOwnProperty('commits') === true) {
        // This was originally to create an array
        // totalCommits.push(githubData[i].payload.commits.length);
...
commitCounter(githubData);

console.log("2. How many of each event type are there? (PullRequestEvent, PushEvent, etc")

// Found resource at https://stackoverflow.com/questions/19395257/how-to-count-duplicate-value-in-an-array-in-javascript
// 4437-4443 creates Array of each event type
let eventTypes = []
...
// Creates empty object to receive key (event type) and number of times it appears in the array eventTypes
let eventCount = {};
...
console.log("3. List all Github users who submitted a pull request that was approved by Steve.")

let steveApproved = [];

for (let i = 0; i < githubData.length; i++) {
//Checks for type of event that is a Pull Request AND action of "closed" AND was merged by Steve
  if (githubData[i].type === 'PullRequestEvent' && githubData[i].payload.action === 'closed' && githubData[i].payload.pull_request.merged_by.login === 'stevebrownlee') {
    //adds user who initiated Pull Request to steveApproved array
    steveApproved.push(githubData[i].payload.pull_request.user.login);
  }
  //ES6 way to sort unique items in array (https://medium.com/front-end-hacking/getting-unique-values-in-javascript-arrays-17063080f836)
  steveApproved = [...new Set(steveApproved)];
...
console.log("4. List all repositories on which Steve had an event, and show how many events were on each one.")

function repoCounter () {
  let repos = [];
  //Loops though each event and adds the repo id# and name to an array
  for (let i=0; i < githubData.length; i++) {
    repos[i] = githubData[i].repo.id + ": " + githubData[i].repo.name;
  }
  //Empty object to hold repo name and # of instances of repo in all events
  let repoCount = {};
  //Counts each unique instance of an item in the array repos[] and adds key:value pair to repoCount{} object
  repos.forEach(function(i) {
    repoCount[i] = (repoCount[i]||0) + 1;
  });
  //Logs each key:value pair in repoCount{}
    console.log(repoCount);
}
repoCounter(githubData);

console.log("5. Which event had the most number of commits?")
//chose to create an array of objects so that the array.sort method could be applied later
let event = [];

for (let i = 0; i < githubData.length; i++) {
  if (githubData[i].payload.hasOwnProperty('commits') === true) {
    //creates an object containing key:value pairs of event id and number of commits per event 
    let obj = {
    "repoId": githubData[i].id,
    "commits": githubData[i].payload.commits.length
    }
    event.push(obj);
  }
}
//creates empty array and begins slicing from beginning of event aray
let mostCommits = event.slice(0);
//compares the commit totals of two ids and sorts by # of commits, greatest to smallest
mostCommits.sort(function(a, b) {
  return b.commits - a.commits;
});
console.log('Most Commits: ');
console.log(mostCommits[0]);

//holy shit that's crazy and awesome and I only partially understand it but I feel like a God in training.

console.log("6. Which programming languages were affected by Steve's events?")

let allLanguages = [];
//Loops over each item in githubData
for (i = 0; i < githubData.length; i++) {
  //Same as the commit filter created earlier. allows filtration down to language property
  if (githubData[i].payload.hasOwnProperty('pull_request') === true) {
    //creates array of all languages. This creates a bunch of empty items in the array...
    allLanguages[i] = githubData[i].payload.pull_request.head.repo.language;
  }
}
//Creates a new variable to hold array that filters out any empty items in the array allLanguages
let languages = allLanguages.filter(function(langName) {
  return langName != null;
})

//creates an array of unique items from the languages array
...
languages.forEach(function(i) {
  langTotals[i] = (langTotals[i]||0) + 1;});

function mostAffected() {
  if (langTotals.Python > langTotals.JavaScript) {
    console.log("The most affected language is Python with " + langTotals.Python + " instances.")
  } else {
    console.log("The most affected language is JavaScript with " + langTotals.JavaScript + " instances.");
  }
}
